    ...
    function findNextTask() {
        // Sort the tasks by priority
         tasks.sort(function(taskA, taskB) {
            if (taskA.get('priority') < taskB.get('priority')) {
                return -1;
            }
        ...
        var task = _.find(tasks, function (task) {
            ...
            var dependencies = task.get('dependencies');
            for (var i = 0; i < dependencies.length; i++) {
                var dependency = dependencies[i];
                if (false) {// TODO: How do we find if the dependency has not been met.
