...
module.exports = function (config) {
    ...
    io.on('connection', function (socket) {
        console.log('Worker connected.  (', findWorkers().length, ')');
        ...
        socket.on('disconnect', function () {
            console.log('Worker disconnected.  (', findWorkers().length, ')');
        ...
        socket.on('error', function (err) {
            console.log({err: err}, "A socket had an error.");
        ...
        socket.on('task:add', function (tasks) {
            // tasks = JSON.parse(tasks); //Socket.io automatically parses JSON strings.
    ...
    function assignTasks() {
        ...
        async.whilst(
            function () {
                ...
                return !_.isEmpty(socket) && existsNextTask;
            },
            function (next) {
                // If there is an available task
                findNextTask(function (task) {
                    existsNextTask = !_.isEmpty(task);
                    if (!existsNextTask) return next();
