...
module.exports = function (config) {
    ...
    db.once('open', function () {
        Task.find(function (err, docs) {
            ...
            for (var i = 0; i < tasks.length; i++) {
                ...
                if (task.assigned && task.assigned.when && task.assigned.when + taskTimeout / 1000 < moment().unix()) {
                    errorTask(task, null, 'Master started with stale task.', 'Task assigned ' + moment.unix(this.task.assigned.when).format());
                }
            }
            tasksLoaded = true;
        });
    ...
    io.on('connection', function (socket) {
        ...
        socket.on('task:done', function (taskJSON) {
            ...
            Log.info({module: task.module}, 'Task completed'); // TODO: Indicate runner name
        ...
        socket.on('task:error', function (err, taskJSON) {
            ...
            var taskParsed = JSON.parse(taskJSON);
            var task = _.find(tasks, "_id", mongoose.Types.ObjectId(taskParsed._id));
            errorTask(task, socket, 'Task error', err)
        });
