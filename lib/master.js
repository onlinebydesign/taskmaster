...
module.exports = function (config) {
    ...
    io.on('connection', function (socket) {
        ...
        socket.on('task:add', function (tasks) {
            ...
            async.each(tasks, function (task, next) {
                ...
                if (err) {
                    socket.emit('task:add:failed');
                    return log.error({err: err, task: task}, 'Error while adding task');
                }
    ...
    function findNextTask(callback) {
        ...
        Task.where('assigned.completed').exists(false)
            .sort('-priority created')
            // Added a limit because there can be millions of unassigned tasks needing attention which can drastically affect performance.
            .limit(1000)
            .exec(function (err, unassignedTasks) {
                ...
                // TODO: Since there is now a limit on the initial tasks called we need to account for not having the full list when checking.
