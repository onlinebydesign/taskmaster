...
module.exports = function (host, port, options) {
    ...
    for (var i = 0; i < options.maxWorkers; i++) {
        ...
        worker.on('task:done', function () {
            socket.emit('task:done', this.task);
        });
         ...
         */
        worker.on('error', function (err, task) {
    ...
    socket.on('task:send', function (taskJSON) {
        ...
        // Add a worker if it will not exceed maxWorkers

        if (workers.length >= options.maxWorkers) {
            socket.emit('error', "All workers are busy.  Can not add another.");
        } else {
